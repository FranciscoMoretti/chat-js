...
import { shouldCopyChatAppFile } from "./scaffold-content";

test("copying an app excludes private environments and generated artifacts at any depth", async () => {
  ...
  try {
    ...
    await cp(source, destination, {
      filter: (file) => shouldCopyChatAppFile(path.relative(source, file)),
      recursive: true,
    });
