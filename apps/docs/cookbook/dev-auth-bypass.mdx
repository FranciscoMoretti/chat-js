...
Have the agent navigate its browser to `/api/dev-login` on the app URL. This creates a dev user session and redirects to the app with valid auth cookies.

## Agent Setup

Use any browser-capable agent or browser automation tool. Navigate to `/api/dev-login` on the app URL before testing authenticated features so that the browser receives the session cookie.

## How It Works

1. Creates a `dev@localhost` user if it doesn't exist
2. Generates a new session with 30-day expiry
3. Sets a signed session cookie using the configured auth cookie name and security attributes
