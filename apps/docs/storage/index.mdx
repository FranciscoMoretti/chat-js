...
application code to one storage provider. Built-in providers use Files SDK adapters. You choose the adapter when you create
...
The installer omits adapters that are not suitable for its generated
production runtime. This includes non-persistent or development-only adapters,
runtime-specific adapters such as Convex and Bun S3, and adapters whose
production credentials must be embedded in constructor options. You can still
integrate those adapters manually when your runtime and secret-management model
support them.
...
You can also select a third-party registry item with a namespace, HTTPS URL,
or local JSON path:
...
The external item must implement the [storage authoring contract](./custom).
Its identifier does not need to be in Files SDK's provider catalog.

## Installed configuration

shadcn installs the selected item source and declared dependencies. The ChatJS
CLI writes typed options and environment requirements without evaluating the item.
...
Credential requirements depend on the selected provider. Built-in items derive
requirements from Files SDK metadata. External items declare their own requirements.
The app validates the generated requirements at build time. Some providers can also use an SDK credential chain such as an IAM
