...
| Vercel   | `VERCEL_APP_CLIENT_ID`     | Vercel App client ID     |
| Vercel   | `VERCEL_APP_CLIENT_SECRET` | Vercel App client secret |
...
<Callout type="note">
  For local development, create a separate GitHub OAuth App with the callback
  URL set to `http://localhost:3000/api/auth/callback/github`.
</Callout>
...
<Callout type="note">
  The Google OAuth consent screen must be configured and published before users
  outside your Google Workspace can sign in. For testing, add yourself as a test
  user without publishing.
</Callout>
...
1. Create a Vercel App using the **[Sign in with Vercel setup guide](https://vercel.com/docs/sign-in-with-vercel/getting-started)**.
2. Under **Redirect URL** add:
   ```
   https://your-domain.com/api/auth/callback/vercel
   ```
3. Copy the **Client ID** and **Client Secret** from the Vercel App.
...
<Callout type="note">
  The Vercel provider is useful when your users are Vercel teams and you want to
  authenticate using their Vercel account. It is off by default.
</Callout>
