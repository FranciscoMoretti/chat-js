...
<Callout type="note">
  ...
  [file storage provider](../storage) for generated images.
</Callout>
...
The previous-image helper recognizes the built-in `{ imageUrl, prompt }` result.
External tools may use different schemas. Unrecognized results are ignored rather
than treated as an image.
