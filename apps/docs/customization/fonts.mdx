...
<Callout type="note">
  Keep the CSS variable names (`--font-geist` and `--font-geist-mono`) the same
  to avoid updating Tailwind config and `globals.css`. If you prefer different
  variable names, update the `fontFamily` section in `tailwind.config.ts` to
  match.
</Callout>
