...
You can deploy the Next.js application and its integrated EVE worker to one Vercel project. Configure a compatible database connection, `AUTH_SECRET`, and OAuth origins before deploying.
...
Configure the credentials listed in [Environment Variables](../reference/env-vars), including `DATABASE_URL` and `AUTH_SECRET`. The integrated worker derives its credential from `AUTH_SECRET` and its origin from `VERCEL_URL`. Its database defaults to `DATABASE_MIGRATION_URL`, then `DATABASE_URL`. Use a direct or session connection; provide a direct URL if the app uses transaction pooling. Leave the three EVE overrides unset unless you need a separate worker, database, or independently rotated secret. The source credentials must be available during the build and at runtime.

Initialize both schemas using the [database setup guide](../reference/database#hosted-workflow-database). Use separate databases for previews and production, with branch-scoped preview credentials.

Production Vercel builds apply application migrations automatically. Preview builds skip them, so run those migrations explicitly before testing a preview. Neither build path provisions the workflow database.
...
Internal EVE calls default to the exact deployment URL (`https://${VERCEL_URL}`), so requests do not move to a newer deployment when a branch alias changes. Keep `EVE_INTERNAL_ORIGIN` unset for this behavior. An explicit override still takes precedence.
