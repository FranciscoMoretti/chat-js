...
For a blank local EVE workflow database, set a local `WORKFLOW_POSTGRES_URL` and run this once before starting the app:

```bash
bun eve:db:setup:local
```

It bootstraps the pinned Workflow World Postgres schema and installs the local lifecycle fences. The command accepts only `localhost` databases. Bootstrap a hosted workflow database with the World Postgres package's documented setup instead; this local command does not establish hosted deletion coverage.
