...
Create application and workflow databases with your provider and set both connection URLs in `.env.local`. Run the same `npm run setup` command. Use a connection with permission to install the workflow provider schema.

The command requires `WORKFLOW_POSTGRES_URL` explicitly, so provider setup cannot fall back to your application database. For loopback hosts, it also installs ChatJS's local lifecycle fences. Hosted setup initializes the provider schema but does not certify hosted deletion or lifecycle coverage.
