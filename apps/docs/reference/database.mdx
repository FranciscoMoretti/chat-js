...
The agent and setup commands share backend selection in `lib/eve/world-config.ts`: managed Workflow on Vercel, PostgreSQL locally/self-hosted. Managed setup skips PostgreSQL initialization. Changing the world requires rebuilding EVE and planning any data migration. The CLI does not offer a world chooser yet.

In the monorepo, these setup commands also work from the repository root. Neither an application build nor a successful connectivity check initializes the workflow database. `eve:check` checks required tables only for PostgreSQL; on Vercel it reports the managed backend without checking SQL tables. Neither check verifies end-to-end lifecycle behavior.
