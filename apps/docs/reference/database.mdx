...
ChatJS uses `DATABASE_URL` for application records everywhere. Its registered-chat workflow backend follows the deployment automatically:

| Runtime | Workflow storage | Configuration |
| --- | --- | --- |
| Vercel Preview or Production | Managed Vercel Workflow | No workflow database URL |
| Local development, tests, or self-hosted Node | PostgreSQL | `WORKFLOW_POSTGRES_URL` |

There is no backend selector to configure. Local Vercel development uses PostgreSQL too. Disposable guest chats retain their isolated local backend during development and use managed Workflow on Vercel. They must not share the registered agent's Graphile queue.

For PostgreSQL workflows, provision a separate database from the application database. Both can share one PostgreSQL server. Application pooling settings do not configure EVE's connection.
...
On Vercel, provision only the application database. EVE uses managed Workflow automatically, even if an old `WORKFLOW_POSTGRES_URL` remains in the deployment environment. Remove that unused setting for clarity. `eve:setup` and `eve:check` report the managed backend and skip workflow SQL setup. They do not certify managed service connectivity.

For a self-hosted, continuously running Node service, create application and workflow databases with your provider and set both connection URLs. Run the same `npm run setup` command with permission to install the workflow provider schema.
