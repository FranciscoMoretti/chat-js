...
The required EVE settings are `EVE_GATEWAY_SECRET`, `WORKFLOW_POSTGRES_URL`, and `EVE_INTERNAL_ORIGIN`, documented [below](#eve-runtime-and-trusted-proxy-headers). In addition to these core settings, **at least one AI gateway credential** and **at least one OAuth provider** must be configured. See the sections below.
...
| `WORKFLOW_POSTGRES_URL` | **Required.** Postgres connection for durable EVE workflows. |
...
`DATABASE_URL` stores application identity and metadata. `WORKFLOW_POSTGRES_URL` stores EVE workflows and native transcripts. Initialize both schemas before sending a message. See the [database setup guide](./database#eve-workflow-database).
...
| `WORKFLOW_POSTGRES_URL` | Required | Postgres connection for durable workflows |
