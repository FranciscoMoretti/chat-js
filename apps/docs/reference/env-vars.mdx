...
adapter compatible with Files SDK. Storage requirements are generated from the selected provider and
...
`lib/storage-options.ts`. See [File storage](../storage) and the
