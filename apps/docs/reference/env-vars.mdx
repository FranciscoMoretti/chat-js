...
The required EVE settings are `EVE_GATEWAY_SECRET` and `EVE_INTERNAL_ORIGIN`. Local and self-hosted runs also require `WORKFLOW_POSTGRES_URL`. Vercel deployments use managed Workflow automatically. These settings are documented [below](#eve-runtime-and-trusted-proxy-headers). In addition to these core settings, **at least one AI gateway credential** and **at least one OAuth provider** must be configured. See the sections below.
...
| Variable | Description |
| --- | --- |
| `EVE_GATEWAY_SECRET` | **Required.** Private credential used between ChatJS and EVE. Generate at least 32 random characters. |
| `WORKFLOW_POSTGRES_URL` | **Local/self-hosted only.** Postgres connection for registered-chat workflows. Unused on Vercel deployments. |
| `EVE_INTERNAL_ORIGIN` | **Required.** Application gateway origin serving `/eve/chat/v1/*`. |
...
`DATABASE_URL` stores application identity and metadata everywhere. Vercel Preview and Production use managed Workflow for EVE execution and transcripts. Local/self-hosted registered chats use `WORKFLOW_POSTGRES_URL` and require both schemas to be initialized. Disposable guest chats keep their isolated local backend during development. See the [database setup guide](./database#eve-workflow-database).

`EVE_INTERNAL_ORIGIN` is independent of `APP_URL`. Set it to the application gateway origin serving the named `/eve/chat/v1/*` mount, without an API route path. A raw EVE worker exposing only `/eve/v1/*` is not a supported endpoint. On protected Vercel deployments, the server forwards `VERCEL_AUTOMATION_BYPASS_SECRET` only when this origin exactly matches `https://${VERCEL_URL}` or `https://${VERCEL_BRANCH_URL}`. It does not forward the credential to other origins, including arbitrary custom domains. The EVE bearer credential and owner identity are still required.
...
| `EVE_GATEWAY_SECRET` | Required | Private ChatJS-to-EVE credential |
| `WORKFLOW_POSTGRES_URL` | EVE | Required locally/self-hosted, unused on Vercel |
| `EVE_INTERNAL_ORIGIN` | Required | Application gateway origin serving the named chat worker |
