...
These variables must be set for the EVE-backed application.
...
The optional EVE overrides and their derived defaults are documented [below](#eve-runtime-and-trusted-proxy-headers). In addition to these core settings, **at least one AI gateway credential** and **at least one OAuth provider** must be configured. See the sections below.
...
`DATABASE_URL` is the runtime connection. Set `DATABASE_MIGRATION_URL` when your host provides a separate direct endpoint for schema operations. It falls back to `DATABASE_URL` when omitted. Set `DATABASE_PREPARE=false` for a transaction pooler without prepared-statement support, and optionally set `DATABASE_MAX_CONNECTIONS` to a positive per-process limit. See [Database](./database) for deployment behavior.

### File Storage

Attachments, image generation, and video generation require a durable storage adapter compatible with Files SDK. Storage requirements are generated from the selected provider and validated by `scripts/check-env.ts`.
...
Provider credentials belong in environment variables or the provider SDK's credential chain. Non-secret adapter options belong in `lib/storage-options.ts`. See [File storage](../storage) and the [Files SDK provider reference](https://files-sdk.dev/docs/providers) for the requirements of your selected provider.

<Callout type="tip">
  When using Vercel Blob, connect a store in your project dashboard under
  **Storage**. `BLOB_READ_WRITE_TOKEN` is added to your environment
  automatically.
</Callout>
...
<Callout type="note">
  ...
  so no additional variables are needed. Set `VERCEL_TEAM_ID`,
  `VERCEL_PROJECT_ID`, and `VERCEL_TOKEN` only when running ChatJS outside of
  Vercel.
</Callout>

---

## EVE runtime and trusted proxy headers

| Variable | Description |
| --- | --- |
| `EVE_GATEWAY_SECRET` | Optional private credential override (at least 32 characters). Defaults to HKDF-SHA256 from `AUTH_SECRET`, with salt `chatjs` and purpose `eve-gateway/v1`. |
| `WORKFLOW_POSTGRES_URL` | Optional workflow database override. Defaults to `DATABASE_MIGRATION_URL`, then `DATABASE_URL`; requires a direct or session connection. |
| `EVE_INTERNAL_ORIGIN` | Optional worker origin override. Defaults to `https://${VERCEL_URL}`, otherwise `APP_URL`, then `http://localhost:${PORT}` (port 3000 by default). |
| `VERCEL_AUTOMATION_BYPASS_SECRET` | Server-only deployment-protection bypass credential for calls to this Vercel project. Required when protection blocks internal EVE requests. |
| `TRUSTED_CLIENT_IP_HEADER` | Optional self-hosted reverse-proxy header that contains one verified client IP. The proxy must overwrite any caller-provided value. Vercel uses its native trusted header. |

Application records and EVE workflows can share a database using separate schemas. An explicit `WORKFLOW_POSTGRES_URL` can select a separate database. Initialize both schemas before sending a message. See the [database setup guide](./database#eve-workflow-database).

Leave `EVE_INTERNAL_ORIGIN` unset for the integrated worker. Vercel internal calls use the exact deployment URL, even when OAuth uses a stable branch alias or `APP_URL`. Outside Vercel, the default follows `APP_URL` or the local port. An explicit origin must not contain a path, query, or credentials. On protected Vercel deployments, the server forwards `VERCEL_AUTOMATION_BYPASS_SECRET` only when this origin exactly matches `https://${VERCEL_URL}` or `https://${VERCEL_BRANCH_URL}`. It does not forward the credential to a separate worker or an arbitrary custom domain. The EVE bearer credential and owner identity are still required.

Rotating `AUTH_SECRET` also rotates the derived EVE credential and invalidates existing guest credentials. Existing explicit EVE secrets are preserved. Separate workers must use the same explicit gateway secret or derive it from the same auth secret. Empty overrides use defaults; malformed non-empty overrides fail validation.
...
| `VERCEL_URL` | Auto-set by Vercel | Deployment hostname, without protocol |
| `VERCEL_BRANCH_URL` | Auto-set by Vercel | Stable branch hostname used in preview environments |
| `VERCEL_ENV` | Auto-set by Vercel | `development`, `preview`, or `production` |

The shared `getBaseUrl()` helper resolves the application origin in this order:

1. Explicit `APP_URL`.
2. `https://${VERCEL_BRANCH_URL}` when `VERCEL_ENV=preview`.
3. `https://${VERCEL_URL}`.
4. `http://localhost:${PORT}` (port 3000 by default).

Better Auth uses `APP_URL` when set. On production Vercel deployments, its fallback is `appUrl` in `chat.config.ts`. Elsewhere, including previews, it uses `getBaseUrl()`. Register OAuth callbacks against the resulting auth origin. Do not carry a local or production `APP_URL` into a preview unless you intend to override its branch URL. EVE uses its separate deployment-first origin precedence described above.
...
| `EVE_GATEWAY_SECRET` | EVE | Optional override; derived from `AUTH_SECRET` |
| `WORKFLOW_POSTGRES_URL` | EVE | Optional direct/session database override |
| `EVE_INTERNAL_ORIGIN` | EVE | Optional worker origin override |
| `TRUSTED_CLIENT_IP_HEADER` | EVE | Optional self-hosted trusted proxy header |
...
| `VERCEL_BRANCH_URL` | Optional | Stable preview branch hostname |
| `VERCEL_ENV` | Optional | Vercel environment classification |
| `VERCEL_AUTOMATION_BYPASS_SECRET` | EVE | Internal access to protected deployments |
...
- [File storage](../storage): provider configuration and credentials
