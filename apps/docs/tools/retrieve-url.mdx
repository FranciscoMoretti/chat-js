...
Interactive creation offers Firecrawl or an external registry item when you enable
URL retrieval. Fresh apps install only the selected retrieval implementation.
Firecrawl’s SDK is installed when the selected retrieval or search item needs it.
...
Your registry item declares `slot: "retrieveUrl"` in `meta.chatjs` and its installed
`chatjs.json` descriptor. Export a standard AI SDK tool with your own input/output
schemas. A renderer is optional. Declare credentials in `envRequirements` so the
CLI checklist and app validation ask for your variables rather than Firecrawl’s.
Only one item can occupy this selection.
