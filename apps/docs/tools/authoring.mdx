...
Export your renderer through `defineToolRenderer`. The wrapper validates persisted
inputs and outputs before calling your component. It also displays failed,
declined, and malformed results. Your component receives typed loading and
...
Both ChatJS chat paths use this registry contract. Custom entries in `custom-ui.ts`
use the same wrapper. Keep runtime imports of server tools out of renderers and
schema modules. You can use type-only imports and `ToolPartFromTool` when extracting
a separate view component. EVE approval requests remain handled by the chat's input
controls; tools with AI SDK approval or model-output policies need an explicit EVE
adapter before they can execute.
...
`tool.ts`, `renderer.tsx`, `schemas.ts`, and `chatjs.json` under `~/tools/chatjs/<id>/`.
