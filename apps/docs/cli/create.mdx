...
| `--search-tool <item>` | Not applicable | Select a search factory registry item. |
...
Run `npx @chat-js/cli@latest sync` after a direct shadcn tool installation. For a recognized `--from-git` ChatJS clone, create replaces its selected gateway, storage, search, and code-execution slots and writes a new configuration.
