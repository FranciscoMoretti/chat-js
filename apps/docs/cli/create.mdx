...
3. **Features:** optional features to enable, followed by provider selection for enabled search, code execution, and URL retrieval tools
...
Run `npx @chat-js/cli@latest sync` after a direct shadcn tool installation. For a recognized `--from-git` ChatJS clone, create replaces its selected gateway, storage, search, code-execution, and URL retrieval slots and writes a new configuration.
Clones must already support `lib/storage-options.ts`. Older ChatJS clones receive
