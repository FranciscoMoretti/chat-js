...
Strict npm installation then exposed peer-version drift hidden by Bun: the vendored EVE archive now declares the microsandbox 0.6 SDK used by the maintained lifecycle integration. The CLI continues to normalize Better Auth versions when producing standalone apps. Peer checks remain enabled; no force/legacy-peer-deps installation workaround is used.
